...
class RWKV(MyModule):
    def __init__(self, model, strategy, verbose = True, convert_and_save_and_exit = None):
        ...
        with torch.no_grad():
            ...
            if not ALREADY_CONVERTED:
                """
                   这里的会尝试在BF16下对Embedding添加LN约束,我这里统一使用FP32
                   这地方造成的数据偏差让我debug好长时间 真的好长!
                """
                #try: # precompute embedding
                #    w['emb.weight'] = F.layer_norm(w['emb.weight'], (args.n_embd,), weight=w['blocks.0.ln0.weight'], bias=w['blocks.0.ln0.bias'],eps=1e-5)
                #
                #except:
                w['emb.weight'] = F.layer_norm(w['emb.weight'].float(), (args.n_embd,), weight=w['blocks.0.ln0.weight'].float(), bias=w['blocks.0.ln0.bias'].float(),eps=1e-5)
                #
    ...
    def att_seq(self, x, sx, aa, bb, pp, ln_w, ln_b, k_mix, v_mix, r_mix, t_decay, t_first, kw, vw, rw, ow, kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry, omx, orx, omy, ory):
        ...
        for t in range(T):
            kk = k[t]
            vv = v[t]
            ww = t_first + kk#t_first = U * E1
            p = torch.maximum(pp, ww)#这里的p作用是计算出最大值,保证e1 e2 exp前每一个元素小于0 #MAX WWt-1,KKt-1,WWt
            e1 = torch.exp(pp - p)# e1 e2同时减去p 相当于除以p 下面的等式的只不受影响
            e2 = torch.exp(ww - p)
            sx[t] = ((e1 * aa + e2 * vv) / (e1 * bb + e2)).to(dtype=x.dtype)
            ww = t_decay + pp

            p = torch.maximum(ww, kk)
            e1 = torch.exp(ww - p) #缩放因子
            e2 = torch.exp(kk - p) #E^K
            aa = e1 * aa + e2 * vv#e1 ww
            bb = e1 * bb + e2
            pp = p
    ...
    def forward(self, tokens, state, full_output=False):
        with torch.no_grad():
            ...
            #print(tokens)
            #print(w['emb.weight'].dtype)


            for i in range(args.n_layer):#只启用一层
                bbb = f'blocks.{i}.'
                att = f'blocks.{i}.att.'
                ffn = f'blocks.{i}.ffn.'
                dd = self.strategy[i]
